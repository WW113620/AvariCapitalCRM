...
namespace AvariCapitalCRM.Models.Utils
{
    ...
    public class StringHelper
    {
        ...
        /// 时间长格式
        /// </summary>
        /// <param name="minute">时长(分钟)</param>
        /// <param name="format">格式</param>
        /// <returns></returns>
        public static string CovertToTime(double minutes, string format = "h", string lang = "zh-hk")
        {

            string timeStr = "";
            string hname = "";
            string mname = "";
            if (lang.ToLower().Trim() == "en-us" || lang.ToLower().Trim() == "eng")
            {
                hname = "h";
                mname = "min";
            }
            ...
            if (minutes > 0)
            {
                if (format.ToLower().Trim() == "h")
                {
                    h = Math.Floor(minutes / 60);
                    m = minutes % 60;                  
                }
            }
            timeStr = h.ToString() + hname + m + mname;
            return timeStr;
        }
        ...
        /// <returns></returns>
        public static string ReplaceString(string sourceStr,Dictionary<string,string> words)
        {
            if (string.IsNullOrWhiteSpace(sourceStr) || words == null || words.Count == 0)
                return sourceStr;
            
            foreach(var word in words)
            {
                sourceStr = sourceStr.Replace(word.Key, word.Value);
            }
